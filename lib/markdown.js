...
const markupContentBlock = content => {
  const filepath = path.join(dirDrafts, content.replace(/^\//, ""));

  if (!fs.existsSync(filepath) || isBinaryFile.sync(filepath)) {
    return `<p>${content}</p>
`;
  }
  ...
  return `<pre><code class="language-${filetype}">${encode(filecontent)}
</code></pre>
`;
};
