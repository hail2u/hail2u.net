...
b.polyfill = function(a, c) {
  if (c) {
    ...
    f = e[a];
    c = c(f);
    c != f && null != c && b.defineProperty(e, a, {configurable:!0, writable:!0, value:c});
  }
};
...
var d = {day:24, hour:60, minute:60, month:30, second:1000, year:12}, g = performance.timing.navigationStart + performance.now(), k = {day:"\u65e5", hour:"\u6642\u9593", minute:"\u5206", month:"\u304b\u6708", now:"\u305f\u3063\u305f\u4eca", second:" \u79d2", year:"\u5e74"};
function l(a) {
  if (Number.isInteger(a) && (a = Math.round((g - a) / d.second), Number.isInteger(a) && !(0 > a))) {
    if (a < d.minute / 2) {
      return k.now;
    }
    if (a < d.minute) {
    ...
    a = Math.round(a / d.minute);
    if (a < d.hour) {
      return a + " " + k.minute + "\u524d";
    }
    a = Math.round(a / d.hour);
    if (a < d.day) {
      return a + " " + k.hour + "\u524d";
    }
    a = Math.round(a / d.day);
    if (a < d.month) {
      return a + " " + k.day + "\u524d";
    }
    a = Math.round(a / d.month);
    if (a < d.year) {
      return a + " " + k.month + "\u524d";
    }
  }
