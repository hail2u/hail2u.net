...
const main = async () => {
  ...
  const template = await fs.readFile(path.join(config.dir.template, "_draft.mustache"), "utf8");
  const rendered = mustache.render(template, selected, null, { escape: escapeCharacters });
