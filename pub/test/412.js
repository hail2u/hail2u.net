    ...
    getProfileIcon: function(name,src) {
        ...
        if (src) {
            ...
            if (this.profileIconType == 'icon' &&
                !/\@/.test(name)) {
                var pre = name.match(/^[\w-]{2}/)[0];
                img.src = (location.protocol == 'https:' ? 'https://www.hatena.com/users/' : 'http://www.st-hatena.com/users/') + pre + '/' + name + '/profile_s.gif';
            } else {
...
Hatena.Star.Entry = new Ten.Class({
    ...
    pushStars: function(s,c) {
        ...
        if (typeof(s[1]) == 'number') {
            this.stars.push(new Hatena.Star.InnerCount(s[1],this,c));
        } else {
            this.stars.push(new Hatena.Star.InnerCount(s.length,this,c));
        }
    },
    ...
    addTemporaryStar: function(args) {
        ...
        var inner_count = Ten.DOM.getElementsByClassName('hatena-star-inner-count', this.star_container);
        inner_count[0].childNodes[0].nodeValue = parseFloat(inner_count[0].childNodes[0].nodeValue) + 1;
    },
...
Hatena.Star.AddButton = new Ten.Class({
    ...
    ImgSrc: Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/add.gif',
...
//      new Ten.Observer(this.img,'onmouseout',this,'hideSelectedColor');
    ...
    addStar: function(e) {
        ...
        var uri = Hatena.Star.BaseURL.replace(/^http:/, '') + this.constructor.AddStarPath + '?uri=' + encodeURIComponent(this.entry.uri) +
...
Hatena.Star.Pallet = new Ten.Class({
    ...
    showSelectedColor: function(pos, addButton) {
        ...
        Hatena.Star.BaseURL.replace(/^http:/, '') + 'colorpalette.selected_color?uri=' + encodeURIComponent(this.addButton.entry.uri) +
    ...
    getPalletFrameURL: function () {
        return Hatena.Star.BaseURL.replace(/^http:/, '') + this.constructor.PALLET_PATH + '?uri=' + encodeURIComponent(this.addButton.entry.uri) + '&location=' + encodeURIComponent(document.location.href);
    },
...
Hatena.Star.AddButton.SmartPhone = new Ten.Class({
    ...
    getPalletFrameURL: function () {
        return Hatena.Star.BaseURL.replace(/^http:/, '') + this.constructor.PALLET_PATH + '?uri=' + encodeURIComponent(this.addButton.entry.uri) + '&location=' + encodeURIComponent(document.location.href) + '&colorscheme=' + this.constructor.getColorScheme();
    }
});
...
Hatena.Star.Pallet.SmartPhone = new Ten.Class({
    ...
    COLOR_SCHEME_DEFINITIONS: {
        dark: {
            ...
            getCloseButtonImage: function () { return Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/close_wh.png' }
        },
        light: {
            ...
            getCloseButtonImage: function () { return Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/close.gif' }
        }
    },
        ...
        this.container.innerHTML =
            '<div id="hatena-star-pallet-container">' + 
                ...
                '<div class="pallet-container">' +
                    '<div class="pallet">' +
                        '<a href="#"><img src="' + Hatena.Star.BaseURL.replace(/^http:/, '') + '/images/spacer.gif" id="hatena-star-yellow" class="star yellow post" alt="Add Yellow Star" title="Add Yellow Star" /></a>' + 
                ...
                '</div>' + 
                '<a href="' + Hatena.Star.PortalURL.replace(/http/, 'https') + '/shop/star?location=' + encodeURIComponent(location.href) + '" id="buy" target="' + (Ten.Browser.isDSi ? '_top' : '_blank' ) + '"><img src="' + Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/buy_star_cart_purple.gif" class="cart">' + Hatena.Star.Text.for_colorstar_shop + '</a>' +
            '</div>' + 
            '<style type="text/css">' +
                ...
                '#hatena-star-pallet-container #hatena-star-yellow {' +
                    'background: url(' + Hatena.Star.BaseURL.replace(/^http:/, '') + (Hatena.Star.Pallet.SmartPhone.ColorScheme == 'dark' ? '/images/add_star_for_smartphone_bk.gif' : '/images/add_star_for_smartphone_wh.gif') + ') 0 0;' +
                    'width: 39px;' +
                    'height: 39px;' +
                    'border: 0;' +
                '}' +
                '#hatena-star-pallet-container a.active #hatena-star-yellow {' +
                    'background: url(' + Hatena.Star.BaseURL.replace(/^http:/, '') + (Hatena.Star.Pallet.SmartPhone.ColorScheme == 'dark' ? '/images/add_star_for_smartphone_bk.gif' : '/images/add_star_for_smartphone_wh.gif') + ') 0 39px;' +
                    'width: 39px;' +
                    'height: 39px;' +
                    'border: 0;' +
                '}' +
...
Hatena.Star.CommentButton = new Ten.Class({
    ...
    ImgSrc: Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/comment.gif'
}, {
    handleKeyUp: function(e) {
        if (!e.isKey('enter')) return;
        var pos = Ten.Geometry.getElementPosition(this.img);
...
Hatena.Star.CommentButtonActive = new Ten.Class({
    ...
    ImgSrc: Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/comment_active.gif'
});
...
//    ImgSrc: Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/star.gif',
    ColorPallet : {
        'yellow' : {
            ImgSrcSelector: '.hatena-star-star-image',
            ImgSrc: Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/star.gif'
        ...
        'green' : {
            ...
            ImgSrc: Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/star-green.gif'
        },
        'red' : {
            ImgSrcSelector: '.hatena-star-red-star-image',
            ImgSrc: Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/star-red.gif'
        },
        'blue' : {
            ImgSrcSelector: '.hatena-star-blue-star-image',
            ImgSrc: Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/star-blue.gif'
        },
        'purple' : {
            ImgSrcSelector: '.hatena-star-purple-star-image',
            ImgSrc: Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/star-purple.gif'
        },
        'temp' : {
            ImgSrcSelector: '.hatena-star-temp-star-image',
            ImgSrc: Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/star-temp.gif'
        }
    }
},{
    ...
    setTimer: function(e) {
        ...
        this.deleteTimer = setTimeout(function() {
            ...
            var uri = Hatena.Star.BaseURL.replace(/^http:/, '') + 'star.deletable.json?name='
    ...
    deleteStar: function() {
        var uri = Hatena.Star.BaseURL.replace(/^http:/, '') + 'star.delete.json?name='
    ...
    getAnchor: function () {
        if (Hatena.Star.isTouchUA) {
            ...
            return Hatena.Star.BaseURL.replace(/^http:/, '') + '/mobile/entry?uri=' + encodeURIComponent(this.entry.uri);
        } else {
...
Hatena.Star.InnerCount = new Ten.Class({
    ...
    showInnerStars: function() {
        var url = Hatena.Star.BaseURL.replace(/^http:/, '') + 'entry.json?uri=' +
...
Hatena.Star.Comment = new Ten.Class({
    ...
    deleteComment: function(callback) {
        ...
        if (!this.id) return;
        if (!Hatena.Visitor.RKS) return;
        var uri = Hatena.Star.BaseURL.replace(/^http:/, '') + 'comment.delete.json?comment_id=' + this.id
            + '&rks=' + Hatena.Visitor.RKS;
...
Hatena.Star.CommentDeleteButton = new Ten.Class({
    initialize: function(parent, comment) {
        ...
        this.button = new Ten.Element('img', {
            src: Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/delete2.gif',
            alt: 'Delete',
            title: 'Delete',
            style: {
                margin: '0 3px',
...
Hatena.LoginWindow = new Ten.Class({
    ...
    hide: function () {
        var script = document.createElement('script');
        script.src = Hatena.Star.BaseURL.replace(/^http:/, '') + 'js/HatenaStar.js';
...
Hatena.Star.CommentScreen = new Ten.Class({
    ...
    getLoadImage: function() {
        var img = document.createElement('img');
        img.src = Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/load.gif';
    ...
    sendComment: function(e) {
        ...
        ci.disabled = 'true';
        this.showLoadImage();
        var url = Hatena.Star.BaseURL.replace(/^http:/, '') + 'comment.add.json?body=' + encodeURIComponent(body) +
...
Hatena.Star.EntryLoader = new Ten.Class({
    ...
    getStarEntries: function() {
        ...
        var url = Hatena.Star.BaseURL.replace(/^http:/, '') + 'entries.json?';
        for (var i = 0; i < entries.length; i++) {
            if (url.length > Ten.JSONP.MaxBytes) {
                new Ten.JSONP(url, c, 'receiveStarEntries');
                url = Hatena.Star.BaseURL.replace(/^http:/, '') + 'entries.json?';
            }
...
Hatena.Star.ConfigLoader = new Ten.Class({
    ...
    loadConfig: function() {
        var uri = Hatena.Star.BaseURL.replace(/^http:/, '') + 'siteconfig.json?host=' + location.hostname;
...
(function () {
    ...
    if (now < valentine_end) {
        Hatena.Star.Star.ColorPallet.yellow.ImgSrc = Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/valentine-star.gif';
        Hatena.Star.Star.ColorPallet.green.ImgSrc  = Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/valentine-star-green.gif';
        Hatena.Star.Star.ColorPallet.red.ImgSrc    = Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/valentine-star-red.gif';
        Hatena.Star.Star.ColorPallet.blue.ImgSrc   = Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/valentine-star-blue.gif';
        Hatena.Star.Star.ColorPallet.purple.ImgSrc = Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/valentine-star-purple.gif';
        Hatena.Star.Star.ColorPallet.temp.ImgSrc   = Hatena.Star.BaseURL.replace(/^http:/, '') + 'images/valentine-star-temp.gif';
    }
})()
} catch (e) {}
